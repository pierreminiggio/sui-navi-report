...
- **`rwa`-market positions now reconstruct correctly, via a static fallback map rather than a generic resolution mechanism.** `getPools()` leaves `contract.reserveId` empty for every `rwa` pool (confirmed: not a different architecture - `rwa` uses the exact same `storage::Storage`/`storage::ReserveData` structs as `main`, just a separate `Storage` object instance per market). `naviHistory.js`'s `KNOWN_RWA_RESERVE_IDS` hardcodes the three reserve addresses discovered so far (USDC, XAUm, XAGm) by inspecting a real supply transaction's object changes - confirmed correct by cross-checking reconstructed XAUm and USDC rwa balances against the live ground-truth report (exact match on XAUm; USDC within expected interest drift). If NAVI lists a new `rwa` asset not in this map, its position is silently skipped (with a console warning) rather than erroring - extending the map means finding that asset's own reserve address the same way (inspect a transaction touching it), since a first attempt at reading the rwa `Storage` object's own dynamic fields generically returned `null` and wasn't pursued further.
