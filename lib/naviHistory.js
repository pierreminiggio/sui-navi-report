...
/**
 * getPools() leaves contract.reserveId empty for every rwa-market pool - it's
 * not a different architecture (rwa uses the exact same storage::Storage /
 * storage::ReserveData structs as main, confirmed by inspecting a real supply
 * transaction), just a gap in what the SDK response exposes. Each entry below
 * is the address of a Field<u8, ReserveData> dynamic-field wrapper object,
 * discovered by inspecting the object changes of a real NAVI supply
 * transaction on the rwa Storage object (0x199c1d5c2d58a4b05bbfa2338d02ad2676572a8a59ac148a5475b5c0fc53ed9f),
 * and confirmed correct by cross-checking reconstructed XAUm/USDC rwa
 * balances against the live ground-truth report.
 *
 * CAVEAT: this is a static snapshot of what one transaction happened to
 * touch, not a generic resolution mechanism - if NAVI lists a new rwa asset
 * this map doesn't know about, that asset's rwa position will be silently
 * skipped (getRwaReserveId returns null, same as "reserve doesn't exist yet"
 * in readReserveAndUserEntries). Extending this map means finding that
 * asset's own reserve wrapper address the same way: inspect a transaction
 * that supplies/borrows it, or query the rwa Storage object's own dynamic
 * fields directly (untested - a first attempt at reading Storage's dynamic
 * field by u8 key returned null, so its real key/table shape is still
 * unconfirmed beyond "these three addresses work").
 */
const KNOWN_RWA_RESERVE_IDS = {
  0: '0xc62d059fa9b8fc6b761011e53da9eaf5546dff3641ce7a76db5f42d46345d655', // USDC
  1: '0xbc6d15e4c44fc0e77e2fe610c6ea5f3fe04649a254a56186834f585dcfe5cf71', // XAUm
  2: '0xded8588df7919375a643f31ecc57a550508224e7b4ed0476ab33f265ed0fba40'  // XAGm
}

function resolveReserveId(pool) {
  if (pool.reserveId) return pool.reserveId
  if (pool.market === 'rwa' && pool.assetId != null) {
    return KNOWN_RWA_RESERVE_IDS[pool.assetId] ?? null
  }
  return null
}
...
export async function reconstructNaviPositionsAt({ address, checkpoint, pools, userKeyBcs }) {
  ...
  for (const pool of pools) {
    const reserveId = resolveReserveId(pool)

    if (!reserveId) {
      console.warn(
        `Skipping ${pool.symbol} (${pool.market}): no resolvable reserveId ` +
        `(getPools() left it empty, and no known fallback address exists for assetId ${pool.assetId})`
      )
      continue
    }
    ...
    const entries = await withRetries(() =>
      readReserveAndUserEntries({ reserveId, checkpoint, userKeyBcs })
    )
